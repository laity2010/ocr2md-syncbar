...
def make_profiles(group, roots):
    gid = safe_profile_component(str(group.get('id') or group.get('name') or 'group'))
    prefs = preferences_for_group(group)
    profiles = []
    hub = roots[0]
    for idx, spoke in enumerate(roots[1:], start=2):
        profile_name = f'ocr2md-group-{gid}-edge-{idx}'
        ...
        profiles.append((profile_name, hub, spoke))
    return profiles
...
def main():
    ...
    plans = []
    for group in groups:
        name, gid, roots = validate_group(group)
        profiles = make_profiles(group, roots)
        for profile_name, _, _ in profiles:
            test_profile(profile_name)
        plans.append((group, name, gid, roots, profiles))

    if args.validate_only:
        for _, name, gid, roots, profiles in plans:
            print(f'OK group={name} id={gid} dirs={len(roots)} edges={len(profiles)}')
    ...
    for _, name, gid, roots, profiles in plans:
        total = len(roots)
        # Two hub-and-spoke sweeps make changes originating at a later spoke
        # reach earlier spokes during the same launch.
        for pass_no in (1, 2):
            for edge_index, (profile_name, hub, spoke) in enumerate(profiles, start=2):
                ...
                current = edge_index - 1 if pass_no == 1 else total
